...
// clang-format off
// Specific formatting of use of `__LINE__' required for correct operation.
 ...
 *  position.
 *  Keep the logic inside the Task method simple for it may cause compile errors.
 ...
 *  \return Returns an indication if the task has yielded (false) or completed (true).
 */
...
/** MACRO: Declare a 'Task' procedure 'name'.
 * _task is a reference to the state variable of the Task.
 *  \return Returns an indication if the task has yielded (false) or completed (true).
 */
#define Task_BeginX(_task, name) \
    bool name() {               \
        bool _yield_ = false;   \
        switch (_task) {        \
            case 0:
 ...
 *  \return Returns an indication if the task has yielded (false) or completed (true).
 */
#define Task_BeginWithParamsX(_task, name, parameters) \
    bool name(parameters)  {                          \
        bool _yield_ = false;                         \
        switch (_task) {                              \
            case 0:
 ...
 *  Exits the task procedure (completed).
 */
#define Task_End \
        }            \
        _task = 0;   \
...
/** MACRO: Exits the Task task procedure immediately (completed).
