...
namespace ATL
{
     ...
     *  \tparam Timeout is specified in the same quantity as the SchedulerT is specified (Milli- or MicroSeconds).
     */
    template <class BaseT, typename SchedulerT, const uint32_t Timeout>
    ...
    public:
        ...
        Task_Begin(Execute)
        {
            Task_YieldUntil(SchedulerT::Wait(BaseT::getId(), Timeout));
            BaseT::OnTimeout();
        }
        Task_End;
